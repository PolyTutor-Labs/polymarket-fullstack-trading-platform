...
class TestKellySizing:
    ...
    def test_elapsed_100pct_applies_time_decay_floor(self):
        # signals.py: when elapsed_pct > 0.7, size *= max(1 - elapsed, 0.30).
        # At 100% elapsed the floor is 30% of untimed Kelly, not zero.
        early = kelly_sizing(0.50, 0.30, elapsed_pct=0.0)
        late = kelly_sizing(0.50, 0.30, elapsed_pct=1.0)
        assert late["kelly_pct"] == pytest.approx(round(early["kelly_pct"] * 0.30, 4))
        assert late["kelly_pct"] > 0


class TestRankBrackets:
    def test_returns_top_n(self):
        # Default top_n is 5 (signals.rank_brackets). All five brackets have +edge.
        ...
        assert len(rank_brackets(probs, prices, top_n=3)) == 3
