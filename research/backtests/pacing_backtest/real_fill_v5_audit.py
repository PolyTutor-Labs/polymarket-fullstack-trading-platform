"""AUDIT dump of real_fill_v5.py — SAME logic, but writes per-auction + per-trade CSVs so every
number in the summary is traceable. Nothing is fabricated: each trade row = a real best-ASK fill
on real L2 depth. Outputs to ./audit_out/. Run: python -u real_fill_v5_audit.py"""
...
OUT=f"{ROOT}/research/backtests/pacing_backtest/audit_out"; os.makedirs(OUT,exist_ok=True)
...
def etstr(u): return datetime.fromtimestamp(u,ET).strftime('%Y-%m-%d %H:%M ET')
def pbk(l):
    l=str(l).strip()
    try:
...
def run(gate):
    ...
    for a in auclist:
        ...
        def rec(st,bkt,ask,stake,fill,sh,won,pnl,note):
            TR.append({'run':'gated' if gate else 'nogate','slug':a['slug'],'dur':a['dur'],'src':a['src'],'strategy':st,
                'bracket':bkt,'entry_ask':round(ask,4),'stake_$':round(stake,2),'fill_$':round(fill,2),
                'shares':round(sh,2),'won':int(won),'pnl_$':round(pnl,2),'note':note})
        def buy(st,bkt,stake,ask,won,dep,note=''):
            fill=min(stake,dep)
            if fill<1 or ask<=0: return
            sh=fill/ask; pnl=sh*((1.0 if won else 0)-ask); R[st]['pnl']+=pnl; R[st]['cap']+=fill
            rec(st,bkt,ask,stake,fill,sh,won,pnl,note)
        pf={b:mid[b] for b in mp if mid.get(b)}
        if pf:
            fav=max(pf,key=lambda b:pf[b][0]); ask=pf[fav][0]; buy('hold_fav',fav,kstake(mp[fav]-ask,ask,KMULT),ask,fav==winner,depth.get(fav,30),'buy favorite bracket at mid-window, hold to resolution')
        ...
        if inplay:
            ...
            for i in range(len(asks)):
                ...
                if pos is None and 0.15<m<0.75 and m<ema[i]-0.04: pos=asks[i]
                    ...
                    fill=min(0.02*BANK,depth.get(inplay,30)); sh=fill/pos; pnl=sh*(bids[i]-pos); R['scrape_sell']['pnl']+=pnl; R['scrape_sell']['cap']+=fill
                    rec('scrape_sell',inplay,pos,0.02*BANK,fill,sh,int(bids[i]>pos),pnl,f'EMA dip-buy @ {pos:.3f} -> sell @ bid {bids[i]:.3f}'); pos=None
        for b in mp:
            ...
            for i in range(2,len(arr2)):
                if arr2[i]<rm[i-1]-0.005 and 0.02<arr2[i]<0.9:
                    fill=min(0.02*BANK,depth.get(b,30)); sh=fill/arr2[i]; pnl=sh*((1.0 if b==winner else 0)-arr2[i]); R['scrape_hold']['pnl']+=pnl; R['scrape_hold']['cap']+=fill
                    rec('scrape_hold',b,arr2[i],0.02*BANK,fill,sh,int(b==winner),pnl,f'blind dip-buy @ {arr2[i]:.3f}, hold to resolution'); break
        for b in [x for x in mp if mp[x]>=0.08]:
            for r in grid:
                if not r.get(b): continue
                ask=r[b][0]; fair=r[b][2]
                if 0.02<ask<fair-0.03 and fair>0.05: buy('kalman_edge',b,kstake(fair-ask,ask,KMULT*calib),ask,b==winner,depth.get(b,30),f'model fair {fair:.3f} > ask {ask:.3f}+0.03, buy & hold'); break
        res.append({'dur':a['dur'],**R})
        prm={b:mid[b][0] for b,_ in brs if mid.get(b)};ptot=sum(prm.values()) or 1
        AD.append({'run':'gated' if gate else 'nogate','slug':a['slug'],'dur':a['dur'],'src':a['src'],
            'win_start_ET':etstr(s),'win_end_ET':etstr(e),'entry_from_ET':etstr(gs),'window_hours':round(total,1),
            'actual_tweet_count':actual,'winner_bracket':winner,'n_brackets':len(brs),
            'n_l2_price_rows':int(len(px)),'n_grid_hours':len(grid),
            'model_implied_prob_winner':round(mp.get(winner,0),3),'market_price_prob_winner':round(prm.get(winner,0)/ptot,3),
            'hold_fav_pnl':round(R['hold_fav']['pnl'],2),'hold_fav_cap':round(R['hold_fav']['cap'],2),
            'scrape_sell_pnl':round(R['scrape_sell']['pnl'],2),'scrape_sell_cap':round(R['scrape_sell']['cap'],2),
            'scrape_hold_pnl':round(R['scrape_hold']['pnl'],2),'scrape_hold_cap':round(R['scrape_hold']['cap'],2),
            'kalman_edge_pnl':round(R['kalman_edge']['pnl'],2),'kalman_edge_cap':round(R['kalman_edge']['cap'],2)})
...
def report(rows,label,out):
    if not rows: out.append((label,0,{})); return
    d={}
    for st in ['hold_fav','scrape_sell','scrape_hold','kalman_edge']:
        cap=sum(r[st]['cap'] for r in rows); pnl=sum(r[st]['pnl'] for r in rows)
        d[st]={'cap':round(cap,2),'pnl':round(pnl,2),'roi':round(100*pnl/cap if cap else 0,1)}
        print(f"    {st:<13}{('$%.0f'%cap):>9}dep {('%+.0f'%pnl):>7}$ ROI {(100*pnl/cap if cap else 0):>+6.1f}%")
    out.append((label,len(rows),d))
SUM=[]; allTR=[]; allAD=[]
for gate,lab in [(True,"WITH 2-days-left gate (Sir's rule)"),(False,"WITHOUT gate")]:
    r,TR,AD=run(gate); allTR+=TR; allAD+=AD
    print(f"\n############ {lab} ############")
    report(r,f"{lab} | ALL",SUM); print("  2-day only:"); report([x for x in r if x['dur']=='2-day'],f"{lab} | 2-day only",SUM)
    print("  7-day only:"); report([x for x in r if x['dur']=='7-day'],f"{lab} | 7-day only",SUM)
pd.DataFrame(allTR).to_csv(f"{OUT}/trades_detail.csv",index=False)
pd.DataFrame(allAD).to_csv(f"{OUT}/auctions_detail.csv",index=False)
srows=[]
for lab,n,d in SUM:
    for st,v in (d or {}).items(): srows.append({'segment':lab,'n_auctions':n,'strategy':st,'capital_deployed_$':v['cap'],'pnl_$':v['pnl'],'roi_pct':v['roi']})
pd.DataFrame(srows).to_csv(f"{OUT}/summary.csv",index=False)
print(f"\nWROTE: {OUT}/trades_detail.csv ({len(allTR)} rows), auctions_detail.csv ({len(allAD)} rows), summary.csv ({len(srows)} rows)")
