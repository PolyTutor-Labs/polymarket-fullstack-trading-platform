...
OUT=ROOT+"/research/backtests/pacing_backtest/audit_out3"; ET=ZoneInfo('America/New_York'); con=duckdb.connect()
...
# tweets + priors
bf=pd.read_parquet(ROOT+"/research/backtests/pacing_backtest/elon_backfill_2025-09_to_now.parquet"); bf=bf[bf.counts_main_feed].sort_values('ms'); pts=(bf.ms.to_numpy()//1000).astype('int64'); c0=int(pts.min())
