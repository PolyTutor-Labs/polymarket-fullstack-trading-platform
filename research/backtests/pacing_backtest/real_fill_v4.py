"""v4: robust slug parser (recovers 7-day auctions) + Sir's LOCKED entry gate (only enter when
<= 48h / 2 days remain, never earlier) + Brier throttle + OOS split. Real best-ASK fills, real
depth caps, Kelly-off-EV sizing. Runs WITH and WITHOUT the 2-days-left gate to show its effect."""
...
def run(gate2d):
    ...
    for _,a in cand.sort_values('start_utc').iterrows():
        ...
        for hr in sorted(px.hr.unique()):
            ...
            if hh>0.97*total or rh<0.5: continue
            if gate2d and rh>48: continue          # LOCKED: only enter with <=2 days left
        ...
        for b in mp:
            ...
            if len(ser)<4 or not(0.05<np.median(ser)<0.9): continue
...
def report(rows,label):
    print(f"  {label} (n={len(rows)}, {sum(r['dur']=='2-day' for r in rows)}x2d {sum(r['dur']=='7-day' for r in rows)}x7d):")
    for st in ['hold_fav','scrape_sell','scrape_hold','kalman_edge']:
        cap=sum(r[st]['cap'] for r in rows); pnl=sum(r[st]['pnl'] for r in rows)
        print(f"    {st:<13}{('$%.0f'%cap):>9} deployed  {('%+.0f'%pnl):>7}$  ROI {(100*pnl/cap if cap else 0):>+6.1f}%")
for gate,lab in [(False,"WITHOUT the 2-days-left gate (enter anytime)"),(True,"WITH Sir's 2-days-left gate (enter only <=48h left)")]:
    res=run(gate); n=len(res); h=n//2
    print(f"\n################  {lab}  ################")
    report(res,"ALL"); report(res[:h],"OOS first half"); report(res[h:],"OOS second half")
