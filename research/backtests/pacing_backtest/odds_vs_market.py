...
ROOT=str(Path(__file__).resolve().parents[3]); CANON=ROOT+"/_DataMetricPulls/canonical"; PMX=ROOT+"/_DataMetricPulls/pmxt_pulled"; ET=ZoneInfo('America/New_York'); con=duckdb.connect()
bf=pd.read_parquet(ROOT+"/research/backtests/pacing_backtest/elon_backfill_2025-09_to_now.parquet"); bf=bf[bf.counts_main_feed].sort_values('ms'); pts=(bf.ms.to_numpy()//1000).astype('int64'); c0=int(pts.min()); c1=int(pts.max())
