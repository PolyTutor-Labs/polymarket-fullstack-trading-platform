...
ROOT=str(Path(__file__).resolve().parents[3]); CANON=ROOT+"/_DataMetricPulls/canonical"; ET=ZoneInfo('America/New_York'); HERE=os.path.dirname(os.path.abspath(__file__)); OUT=HERE+"/audit_out3"
bf=pd.read_parquet(ROOT+"/research/backtests/pacing_backtest/elon_backfill_2025-09_to_now.parquet"); bf=bf[bf.counts_main_feed]; c1=int(bf.ms.max()//1000)
