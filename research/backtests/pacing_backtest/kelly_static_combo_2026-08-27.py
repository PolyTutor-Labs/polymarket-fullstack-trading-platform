...
`research/backtests/pacing_backtest/prereg/kelly_static_combo_2026-08-27.md` (BINDING,
...
def run_group(handle, dur, wall, borrowed_config=None):
    ...
    b2_note = (
        "*** HINDSIGHT-SELECTED (TRAIN=0 -> picked using the FULL population, including "
        "sealed/future auctions -- NOT achievable live; see B2_walkforward for the honest "
        "companion) ***"
        if b2_hindsight
        else f"(TRAIN-selected, n_train={len(train_idx)})"
    )
    print(f"  B2 baseline bracket: {b2_bracket!r}  {b2_note}")
...
def build_wh_descriptive():
    wh_post_path = ROOT / "research" / "backtests" / "pacing_backtest" / "wh_backfill_2026-06_to_07.parquet"
...
def main():
    ...
    emit_run_meta(
        ...
        data_paths=[str(CANON / "auctions" / "elonmusk"), str(CANON / "auctions" / "realDonaldTrump"),
                    ...
                    str(ROOT / "research" / "backtests" / "pacing_backtest" / "wh_backfill_2026-06_to_07.parquet"),
