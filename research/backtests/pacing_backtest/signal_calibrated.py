# -*- coding: utf-8 -*-
"""YOUR divergence strategy, SIGNALS ONLY (no fills, no P&L), on the SAME auction (may-14-may-16),
now using the CALIBRATED pace model instead of raw AccrualCurve:
  our pace = time-weighted ENSEMBLE of Kalman (early) + AccrualCurve (late)  [the sheet spec]
  market   = reverse-pace (sum bracket-center x live price)
  edge     = divergence; BUY the bracket our pace points to, rotate as it moves, close on converge.
Compare the trade count + projections to the raw-AccrualCurve run (33 trades, projections 110-300)."""
...
OUT=f"{ROOT}/research/backtests/pacing_backtest/audit_out3"; os.makedirs(OUT,exist_ok=True)
...
for _,a in auc.iterrows():
    ...
    if not w: continue
    allres.append({'s':w[0],'e':w[1],'final':obs(*w)})
...
winner=str(row.winning_bucket); priors=[p['final']/((p['e']-p['s'])/3600) for p in allres if p['e']<s and p['e']>p['s']]
share=share_wf(48,s)
print(f"AUCTION {SLUG} | winner {winner} | final {obs(s,e)} | {len(priors)} priors for Kalman")
def l2files(s,e):
...
for bar in grid.index:
    t=bar/1000.0; elapsed=(t-s)/3600.0; ef=elapsed/total
    if elapsed<3 or elapsed>total-0.3: continue
    o=obs(s,int(t)); rh=total-elapsed
    proj_k=o+kalman_rate(o,elapsed,priors)*rh
    idx=min(len(share)-1,max(0,int(elapsed)-1)); proj_a=o/share[idx]
    our=(1-ef)*proj_k+ef*proj_a
    ...
    def rec(act,b,px_,reason):
        trades.append({'action':act,'bracket':b,'et':datetime.fromtimestamp(t,ET).strftime('%m-%d %H:%M'),'tweets':o,
            'our_count':round(our,1),'kalman':round(proj_k,1),'accrual':round(proj_a,1),'mkt_count':round(mkt,1),
    ...
    if pos is None:
        if abs(div)>=ENTER and target and target in prices:
            pos={'b':target}; action=f"BUY {target}"; rec('BUY',target,prices[target],f"our pace {our:.0f} vs market {mkt:.0f}, edge {abs(div):.0f}")
    else:
        held=pos['b']; hp=prices.get(held,float('nan'))
        if abs(div)<=EXIT or target!=held:
            action=f"SELL {held}"; rec('SELL',held,hp,('divergence converged (<=%.0f)'%EXIT) if abs(div)<=EXIT else f"pace moved to {target}"); pos=None
    ...
    rows.append({'et':datetime.fromtimestamp(t,ET).strftime('%m-%d %H:%M'),'elapsed_h':round(elapsed,1),'tweets':o,
        'kalman':round(proj_k,1),'accrual':round(proj_a,1),'ensemble(our)':round(our,1),'mkt(revpace)':round(mkt,1),
...
td.to_csv(f"{OUT}/signals_calibrated.csv",index=False); series.to_csv(f"{OUT}/timeline_calibrated.csv",index=False)
nb=int((td.action=='BUY').sum()) if len(td) else 0; ns=int((td.action=='SELL').sum()) if len(td) else 0
print(f"\nCALIBRATED ENSEMBLE: {len(td)} signals ({nb} BUY, {ns} SELL)  [raw AccrualCurve gave 33]")
print(f"our ensemble projection range: {series['ensemble(our)'].min():.0f} - {series['ensemble(our)'].max():.0f}  (raw AccrualCurve was 110-311; actual final {obs(s,e)})")
print(f"market reverse-pace range: {series['mkt(revpace)'].min():.0f} - {series['mkt(revpace)'].max():.0f}")
print(f"divergence range: {series.divergence.min():+.0f} to {series.divergence.max():+.0f}  (|div|>=5 = trade)")
if len(td):
    print("\nall signals:")
    for _,r in td.iterrows():
        print(f"  {r.et} | {r.action:<9} | ens {r.our_count:>5} (K {r.kalman:>4}/A {r.accrual:>5}) vs mkt {r.mkt_count:>5} | div {r.divergence:>+5} | px {r.price} | {r.reason}")
print(f"\nWROTE {OUT}/signals_calibrated.csv + timeline_calibrated.csv")
