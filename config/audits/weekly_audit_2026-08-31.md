...
- Dead: `tests/test_engine.py`, `tests/test_copy_trading.py` reference removed classes/modules. `config/spike_trading_module_spec.md` specs a module that no longer exists. `HANDOFF_DELETE_BOT.md` (repo root, from an apparently-abandoned 2026-06-15 decommission attempt) is stale clutter now that the bot is clearly still live and being rebuilt as `feat/newbot-step1-skeleton`.
