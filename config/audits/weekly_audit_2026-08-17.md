...
`config/MODULE_ARCHITECTURE.md` — the doc `CLAUDE.md` names as "the full guide" for module rules — describes `truth_social/`, `elon_tweets/`, `spike_trading/`, and `copy_trading/` as the module directories, with example files like `parquet_history.py`, `lunarcrush.py`, `decision.py` under `spike_trading/`.
