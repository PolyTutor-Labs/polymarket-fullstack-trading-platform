...
- config/ — detailed architecture, strategy, features, API docs
...
- NEVER modify position limits without updating config/STRATEGY.md + risk-rules.md
...
See `config/MODULE_ARCHITECTURE.md` for the full guide. Quick rules:
...
- `@qa-architecture-quality` — module boundaries, abstraction leaks, dependency direction, convention compliance vs AGENTS.md. Run monthly or before adding a new module.
...
## Autonomy
- **Use Codex in Chrome and Codex Preview MCP tools proactively whenever the task is browser-actionable.** This includes Railway deploys, redeploys, dashboard checks, deploy log inspection, and any other web-based action that would otherwise require user clicks. Do not ask the user to "open the dashboard" or "click redeploy" — drive the browser via MCP and report results.
...
1. **`config/lessons.md`** — human-readable changelog of what went wrong (full narrative)
2. **`~/.Codex/projects/<this-repo>/memory/lesson_<slug>.md`** — auto-loaded memory file with the RULE (concise, actionable). Then add a one-line pointer to `memory/MEMORY.md` under the "Hard-won lessons" section.
